...
try {
	...
	if (z == 1) {
		st.executeUpdate("update cart set total = '" + cart_total + "',quanitity = '" + quantity
		+ "' where product_id = '" + productid + "' and email = '" + email + "' and address is NULL");
		response.sendRedirect("home.jsp?msg=exist");
	}
	if (z == 0) {
		PreparedStatement ps = con
		.prepareStatement("insert into cart(email,product_id,quanitity,price,total) values (?,?,?,?,?)");
